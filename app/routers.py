...
def create_client() -> FlaskResponse | tuple[FlaskResponse, int]:
...
def get_client(client_id: int) -> FlaskResponse | tuple[FlaskResponse, int]:
    client = Client.query.get_or_404(client_id)
    return jsonify({"id": client.id, "name": client.name})


@bp.route("/parkings", methods=["POST"])
def create_parking() -> FlaskResponse | tuple[FlaskResponse, int]:
    data: Dict[str, Any] = request.get_json()
...
def get_parking(parking_id: int) -> FlaskResponse | tuple[FlaskResponse, int]:
    parking = Parking.query.get_or_404(parking_id)
    return jsonify({"id": parking.id, "location": parking.location})


@bp.route("/assign", methods=["POST"])
def assign_client_to_parking() -> FlaskResponse | tuple[FlaskResponse, int]:
    data: Dict[str, Any] = request.get_json()
