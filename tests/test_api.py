...
def test_create_client(client):
    response = client.post("/clients", json={"name": "Test", "surname": "User", "credit_card": "1111", "car_number": "B222BB"})
    assert response.status_code == 201

def test_create_parking(client):
    response = client.post("/parkings", json={"address": "Lenina 1", "opened": True, "count_places": 10, "count_available_places": 10})
    assert response.status_code == 201
...
def test_exit_parking(client, test_data):
    client_obj, parking_obj = test_data
    client.post("/client_parkings", json={"client_id": client_obj.id, "parking_id": parking_obj.id})
